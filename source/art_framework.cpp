...
void ART::generate_read_with_indel(int threadID){
    long start_position = static_cast<long>(floor(rng::rand_double(0,1,threadID)*valid_region));        // Randomly selects a starting position on the reference sequence within a valid region 
...
void ART::get_read_quality(std::vector<short>& read_quality_vec, int read_number, int threadID){
    ...
    for(int i=0; i<read_length; i++){                                                                   // For each position of the read
        cumCC = static_cast<int>(ceil(rng::rand_double(0,1,threadID)*1000000));                         // Randomly get a map key
...
void ART::generate_paired_reads_with_indel(ART& read_1, ART& read_2, int minFragSize, std::vector<double>& fragment_weights, int threadID){
    ...
    long start_position_1 = static_cast<long>(floor((chromSegment_length-fragment_length)*rng::rand_double(0,1, threadID))); // Starting position for read 1
    long start_position_2 = (start_position_1+fragment_length)-(read_length-length_changed_2);            // This would be the starting position for read 2 in forward strand
