...
private:
    ...
    std::vector<std::vector<std::vector<long>>> dsb_fragments_left;                  // Stores left DSB fragments, one entry per fragment. Each entry is {blunted_end_location, other_end_location, chromosome_index, causing_dsb_strand1_location, causing_dsb_strand2_location}; for left fragments element 0 > element 1 (see load_dsb_fragments)
    std::vector<std::vector<std::vector<long>>> dsb_fragments_right;                 // Stores right DSB fragments, in the same per-entry format as dsb_fragments_left, except element 1 > element 0 (see load_dsb_fragments)
    std::vector<std::vector<std::vector<long>>> dsb_strands_left;                    // Stores left DSB strands that survived single-strand-damage filtering, one entry per strand, copied through unchanged from the corresponding dsb_fragments_left entry (same per-entry format)
    std::vector<std::vector<std::vector<long>>> dsb_strands_right;                   // Stores right DSB strands that survived single-strand-damage filtering, one entry per strand, copied through unchanged from the corresponding dsb_fragments_right entry (same per-entry format)
    std::vector<std::vector<std::vector<long>>> base_pair_damages_left;              // Stores base pair damages on the left DSB strands. Indexed the same as dsb_strands_left: entry i is the (possibly empty) list of base-pair damage locations found on dsb_strands_left's i-th strand
    std::vector<std::vector<std::vector<long>>> base_pair_damages_right;             // Stores base pair damages on the right DSB strands. Indexed the same as dsb_strands_right: entry i is the (possibly empty) list of base-pair damage locations found on dsb_strands_right's i-th strand
